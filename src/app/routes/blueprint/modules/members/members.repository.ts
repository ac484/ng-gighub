...
export class MembersRepository {
  ...
  async findByBlueprint(blueprintId: string): Promise<Member[]> {
    try {
      ...
      this.logError('findByBlueprint', error, { blueprintId });
      throw error;
    }
  }
  ...
  async addMember(request: CreateMemberRequest): Promise<Member> {
    ...
    try {
      ...
      this.logError('addMember', error, { blueprintId: request.blueprintId });
      throw error;
    }
  }
  ...
  async updateMember(blueprintId: string, memberId: string, payload: UpdateMemberRequest): Promise<void> {
    try {
      ...
      this.logError('updateMember', error, { blueprintId, memberId });
      throw error;
    }
  }
  ...
  async removeMember(blueprintId: string, memberId: string): Promise<void> {
    try {
      ...
      this.logError('removeMember', error, { blueprintId, memberId });
      throw error;
    }
  }
  ...
  private validateMemberType(ownerType: OwnerType, memberType: BlueprintMemberType, isExternal: boolean): void {
    ...
    if (!allowed.includes(memberType)) {
      ...
      this.logError('validateMemberType', error, { ownerType, memberType });
      throw error;
    }
  ...
  logError(action: string, error: unknown, context?: Record<string, unknown>): void {
    this.logger.error('[MembersRepository]', action, error as Error, context);
  }
}

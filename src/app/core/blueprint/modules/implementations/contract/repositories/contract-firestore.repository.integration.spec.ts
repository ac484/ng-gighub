...
import { connectFirestoreToEmulator, disconnectFromEmulators, EMULATOR_CONFIG } from '@test/firebase-emulator.setup';
import { createTestContract, createTestContracts, waitFor, generateTestId } from '@test/test-data.factory';
...
describe('ContractRepository Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    console.log('[Test] Firebase Emulator connected:', `${EMULATOR_CONFIG.firestore.host}:${EMULATOR_CONFIG.firestore.port}`);
  });
  ...
  describe('Real-time Subscriptions', () => {
    it('should subscribe to contract changes', async () => {
      ...
      const subscription = repository.findById(testBlueprintId, contractId).subscribe(updated => {
        if (updated) {
    ...
    it('should subscribe to collection changes', async () => {
      ...
      const subscription = repository.findAll(testBlueprintId).subscribe(contracts => {
  ...
  describe('Query Operations', () => {
    ...
    it('should handle non-existent contract', async () => {
      // Act
      const nonExistent = await firstValueFrom(repository.findById(testBlueprintId, 'non-existent-id'));

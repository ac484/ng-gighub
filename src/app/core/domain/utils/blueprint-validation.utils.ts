 ...
 * ```
 */
export function isValidMemberTypeForOwner(
  ownerType: OwnerType,
 ...
 * ```
 */
export function isValidAssigneeTypeForOwner(
  ownerType: OwnerType,
...
export function validateTaskAssignment(
  ...
  if (!isValidAssigneeTypeForOwner(ownerType, assigneeType)) {
    ...
    const allowedTypesText = allowedTypes
      .map(type => {
        switch (type) {
          case AssigneeType.USER: return '用戶';
          case AssigneeType.TEAM: return '團隊';
          case AssigneeType.PARTNER: return '夥伴';
          default: return type;
        }
      })
      .join('、');
    ...
    return {
      isValid: false,
      error: ownerType === OwnerType.USER
        ? '個人藍圖只能指派給用戶'
        : `此藍圖只能指派給：${allowedTypesText}`
    };
  }

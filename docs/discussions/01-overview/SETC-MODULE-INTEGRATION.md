...
1. **零直接依賴**: 模組間嚴禁直接注入其他模組的服務
2. **事件驅動通訊**: 所有模組間通訊必須透過 BlueprintEventBus
3. **上下文隔離**: 每個 Blueprint 有獨立的事件總線與資料範圍
4. **自動化流程**: 透過事件訂閱實現業務流程自動化

### Blueprint Context 架構層級

```mermaid
graph TB
    subgraph "Global Layer 全域層"
        User[User Context<br/>使用者上下文]
        Org[Organization Context<br/>組織上下文]
    end
    
    subgraph "Blueprint Layer 藍圖層 ⭐"
        BC[Blueprint Context<br/>藍圖上下文]
        EB[Blueprint Event Bus<br/>事件總線]
        PM[Permission Service<br/>權限服務]
        
        BC -->|provides| EB
        BC -->|provides| PM
    end
    
    subgraph "Module Layer 模組層"
    ...
    BC -->|provides context| Contract
    BC -->|provides context| Task
    BC -->|provides context| Log
    BC -->|provides context| QC
    BC -->|provides context| Defect
    BC -->|provides context| Acceptance
    BC -->|provides context| Issue
    BC -->|provides context| Warranty
    BC -->|provides context| Finance
    
    Contract -.->|emit/subscribe| EB
    Task -.->|emit/subscribe| EB
    Log -.->|emit/subscribe| EB
    QC -.->|emit/subscribe| EB
    Defect -.->|emit/subscribe| EB
    Acceptance -.->|emit/subscribe| EB
    Issue -.->|emit/subscribe| EB
    Warranty -.->|emit/subscribe| EB
    Finance -.->|emit/subscribe| EB
    ...
    style Org fill:#ddf,stroke:#333,stroke-width:2px
```

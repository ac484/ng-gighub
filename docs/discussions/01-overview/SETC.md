...
## 🏗️ Blueprint Event Bus 架構 (MANDATORY)

### 🚨 核心架構原則

**所有模組間通訊必須透過 BlueprintEventBus，嚴禁直接依賴**

```mermaid
graph TB
    subgraph "Blueprint Context 藍圖上下文"
        EB[BlueprintEventBus<br/>事件總線]
        
        subgraph "Phase 0: Contract 合約"
            Contract[Contract Module]
        end
        
        subgraph "Phase 1: Task 任務"
            Task[Task Module]
        end
        
        subgraph "Phase 2: Quality & Acceptance 品質驗收"
            Log[Log Module]
            QC[QC Module]
    ...
    Invoice -->|emit events| EB
    Payment -->|emit events| EB
    ...
    EB -->|subscribe| Invoice
    EB -->|subscribe| Payment
...
Log Module 自動建立日誌
    ↓ (event)
QC Module 自動建立 QC 待驗
    ↓
QC.passed / QC.failed
    ↓ (event, branch)
    ├─ passed → Acceptance Module
    └─ failed → Defect Module
    ↓
Acceptance.passed / Acceptance.failed
    ↓ (event, branch)
    ├─ passed → Invoice + Warranty (parallel)
    └─ failed → Issue Module
```
...
export class ModuleA {
  private moduleB = inject(ModuleBService);  // ❌ 跨模組注入
  
  doSomething() {
    this.moduleB.doAction();  // ❌ 直接呼叫
  }
}
```
...
詳細整合說明請參考: [SETC-MODULE-INTEGRATION.md](./SETC-MODULE-INTEGRATION.md)

 ...
 */
export type RetryableErrorCode = 'UNAVAILABLE' | 'DEADLINE_EXCEEDED' | 'RESOURCE_EXHAUSTED' | 'INTERNAL';
...
export function isRetryableError(error: any): boolean {
  const retryableCodes: RetryableErrorCode[] = ['UNAVAILABLE', 'DEADLINE_EXCEEDED', 'RESOURCE_EXHAUSTED', 'INTERNAL'];
...
export function calculateOverallConfidence(scores: ConfidenceScores): number {
  const values = [scores.contractNumber, scores.title, scores.totalAmount, scores.dates, scores.parties, scores.workItems];
